...
1. All OAuth protocol endpoints and resources, including authorization server endpoints, MCP resource server endpoints, Protected Resource Metadata, authorization server metadata, and client registration endpoints **MUST** be served over HTTPS.
